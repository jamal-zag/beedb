...
AddToIndexOperator::AddToIndexOperator(beedb::concurrency::Transaction *transaction, const std::uint32_t column_index,
                                       const std::shared_ptr<index::IndexInterface> index)
    : UnaryOperator(transaction), _column_index(column_index), _index(index)
{
}
...
beedb::util::optional<beedb::table::Tuple> AddToIndexOperator::next()
{
    ...
    if (next == true)
    {
        if (next->page_id() != storage::Page::INVALID_PAGE_ID)
        {
