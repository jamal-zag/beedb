...
ProjectionOperator::ProjectionOperator(beedb::concurrency::Transaction *transaction, table::Schema &&schema)
    : UnaryOperator(transaction), _schema(std::move(schema))
{
}
...
beedb::util::optional<beedb::table::Tuple> ProjectionOperator::next()
{
    ...
    if (next_tuple == true)
    {
        return util::optional{table::Tuple(this->_schema, std::move(next_tuple.value()))};
    }
